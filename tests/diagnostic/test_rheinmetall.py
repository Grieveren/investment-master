"""
Simple test to verify we can fetch Rheinmetall data from SimplyWall.st API.
"""
...
def test_rheinmetall_fetch():
    """Test that we can fetch Rheinmetall data from SimplyWall.st API."""
    ...
    # Fetch Rheinmetall data
    # Try both common exchange symbols
    ticker = "RHM"
    exchanges = ["XTRA", "ETR"]
    ...
    for exchange in exchanges:
        ...
        if result:
            ...
            print(f"❌ ERROR: Failed to fetch data for {ticker} on {exchange}")
    
    # If we get here, both exchanges failed
    print("❌ ERROR: Failed to fetch data for Rheinmetall on all tried exchanges")
    return False

if __name__ == "__main__":
    success = test_rheinmetall_fetch()
